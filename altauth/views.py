    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.cleaned_data['passphrase'] = (
                form.cleaned_data['passphrase'] or get_random_string(100))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            pb_entry, created = PublicKey.objects.get_or_create(
                user=request.user, pubkey_type=form.cleaned_data['pubkey_type'])
...
def get_public_key_token(request, pubkey_type='RSA'):
    ...
    authentication.
    The returned value is a text/plain content encripted with the user's public
    key: <2-digits token length > <token> <server public key>
    """
    if request.method == 'POST' and request.POST['username']:
        try:
            user = User.objects.get(username = request.POST['username'])
            public_key = PublicKey.objects.get(
                user=user, pubkey_type=pubkey_type)
        except PublicKey.DoesNotExist:
            return HttpResponseForbidden(
                'no public key available for pubkey type %(pubkey_type)s' %
                {'pubkey_type': pubkey_type}
            )
        except User.DoesNotExist:
            return HttpResponseForbidden(
                "Invalid username: %(username)s") % {
                    'username': request.POST['username']}

        login_token = public_key.generate_login_token()
        return HttpResponse(login_token, content_type='text/plain')
    else:
        return HttpResponseForbidden("username not specified")
