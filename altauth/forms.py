...
class SetPublicKeyForm(forms.Form):
    ...
    """
    public_key = forms.CharField(widget=forms.Textarea, max_length=500,
                                 required=True)
    pubkey_type = forms.ChoiceField(
        choices=ALLOWED_PUBLICKEY_TYPES, required=False, initial='RSA')
...
class AlternativePasswordLoginForm(AltauthLoginForm):
    ...
    def clean(self):
        ...
        if username and alternative_password:
            self.user_cache = authenticate(
                username=username, alternative_password=alternative_password)
